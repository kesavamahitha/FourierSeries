...
Consider,
\begin{align}
	x(t) &= \sum_{n=-\infty}^{\infty} c_{n}e^{jnf t}
	\label{eq:compfs}
\end{align}
where $c_{n}$ is the exponential fourier coefficient.
\begin{align}
        c_{n} &= \frac{1}{T} \int_{0}^{T} x(t) e^{-jnf t} \, dt
\end{align}
where $T$ is the time period of function $x(t)$.
\item Trigonometric Fourier Series\\
We can write:
\begin{align}
	e^{jnf t} = cos(nf t) + j sin(nf t)
	\label{eq:expfs}
\end{align}
Substituting \eqref{eq:expfs} in \eqref{eq:compfs}
\begin{align}
	 x(t) &= \sum_{n=-\infty}^{\infty} c_{n} (\cos(nf t) + j \sin(nf t))\\
	      &= a_0 + \sum_{n=1}^{\infty} (a_{n} \cos(nf t) + b_{n} \sin(nf t))
\end{align}
...
\begin{align}
          ...
          a_{n} &= 2Re(c_{n})\\
                &=\frac{2}{T} \int_{0}^{T} x(t)\cos(nf t) \, dt\\
          b_{n} &= -2Im(c_{n})\\
                &= \frac{2}{T} \int_{0}^{T} x(t)\sin(nf t) \, dt
\end{align}
\end{enumerate}
\end{document}
